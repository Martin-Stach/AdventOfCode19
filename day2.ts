...
export function day2() {
    ...
    const inputFileContents = fs.readFileSync("day2.txt", 'utf-8');
    ...
    const inputArray : number[] = inputFileContents.split(',').map(Number);
    // modules.pop();

    // console.log(modules);

    for (let index = 0; index < inputArray.length; index += 4) {
        console.log("current loop index: " + index + " with value: " + inputArray[index]);

        const opcode : number = inputArray[index];
        switch (opcode) {
            case 99:
